document.addEventListener("DOMContentLoaded", async function () {
  ...
  async function aufgabenLaden() {
    try {
      ...
      const response = await fetch(
        `https://nd5tbhnyvl.execute-api.eu-central-1.amazonaws.com/stage/all_task?listName=${listName}&UserName=${userName}`
      );
      if (!response.ok) {
  ...
  async function aufgabeLoeschen(listName, taskDescription) {
    try {
      ...
      const response = await fetch(
        `https://nd5tbhnyvl.execute-api.eu-central-1.amazonaws.com/stage/delete_task`,
        {
          method: "POST",
  ...
  async function showTableNames() {
    try {
      const userName = document.getElementById("userIdInput").value;
      const apiUrl =
        "https://nd5tbhnyvl.execute-api.eu-central-1.amazonaws.com/st/listName_post";
  ...
  function renderTableNames(tableNames) {
    ...
    tableNames.forEach((tableName) => {
      ...
      li.addEventListener("click", function () {
        document.querySelectorAll("#tableList li").forEach(function (item) {
  ...
  async function deleteTable(listName) {
    try {
      ...
      const confirmed = confirm(
        "Are you sure you want to delete this list with all its tasks?"
      );
      if (!confirmed) return;
      ...
      const response = await fetch(
        "https://nd5tbhnyvl.execute-api.eu-central-1.amazonaws.com/stage/delete_listname",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  async function aufgabeHinzufuegen() {
    try {
      ...
      const response = await fetch(
        "https://nd5tbhnyvl.execute-api.eu-central-1.amazonaws.com/st/post_item",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(requestBody),
  ...
  async function updateTaskStatus(taskDescription, checked) {
    const listName = document.getElementById("listNameInput").value;
    const response = await fetch(
      `https://nd5tbhnyvl.execute-api.eu-central-1.amazonaws.com/stage/checked`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
